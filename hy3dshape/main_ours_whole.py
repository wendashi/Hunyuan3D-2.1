...
def load_converted_checkpoint(model: pl.LightningModule, ckpt_dir: str) -> bool:
    if not ckpt_dir or not os.path.isdir(ckpt_dir):
        return False

    index_path = os.path.join(ckpt_dir, "pytorch_model.bin.index.json")
    single_path = os.path.join(ckpt_dir, "pytorch_model.bin")
    ...
    if os.path.exists(index_path):
        ...
        for param_name, shard_file in index.get("weight_map", {}).items():
            ...
            if shard_file not in shard_cache:
                shard_cache[shard_file] = torch.load(shard_path, map_location="cpu")
            state_dict[param_name] = shard_cache[shard_file][param_name]
    elif os.path.exists(single_path):
        state_dict = torch.load(single_path, map_location="cpu")
    else:
        return False

    rank_zero_info(f"Loading converted checkpoint from {ckpt_dir}")
    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    if missing:
        rank_zero_info(f"Missing keys when loading converted checkpoint: {missing}")
    if unexpected:
        rank_zero_info(f"Unexpected keys when loading converted checkpoint: {unexpected}")

    forward_module = getattr(model, "_forward_module", None)
    if forward_module is not None:
        for name, param in forward_module.named_parameters():
            if name.startswith("first_stage_model") or name.startswith("cond_stage_model"):
...
if __name__ == "__main__":
    ...
    manual_loaded = load_converted_checkpoint(model, training_cfg.get('ckpt_path', None))
    ...
    # 根据 val_check_interval 是否存在来决定是否禁用 sanity check
    val_check_interval = training_cfg.get('val_check_interval')
    # 如果 val_check_interval 不存在，禁用 sanity check；否则使用默认值 2
    num_sanity_val_steps = 0 if val_check_interval is None else 2
    ...
    trainer = pl.Trainer(
        ...
        num_sanity_val_steps=num_sanity_val_steps  # 如果 val_check_interval 不存在，则禁用 sanity check
    )

    # Train
    if manual_loaded:
        rank_zero_info("Converted checkpoint loaded, optimizer states will restart from scratch.")
        training_cfg.ckpt_path = None
    elif training_cfg.ckpt_path == '':
        training_cfg.ckpt_path = None
