...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Render multiple models with single front view")
...
def make_single_args(batch_args, model_path: str, out_dir: str):
    ...
    # render.py 需要的参数集合 - 单视图版本固定为1个视图
...
def rename_rendered_files(out_dir: str, input_filename: str):
    """重命名渲染后的文件，将000_xxx格式改为input_filename_xxx格式"""
    import shutil
    
    # 定义需要重命名的文件模式（单视图只保存主图）
    file_patterns = [
        ('000.png', f'{input_filename}.png')
    ]
    
    for old_name, new_name in file_patterns:
        ...
        if os.path.exists(old_path):
            try:
                ...
                print(f"[WARN] Failed to rename {old_name}: {e}")
        else:
            print(f"[DEBUG] File not found: {old_name}")
...
## 渲染参数
- **分辨率**: {os.environ.get('RESOLUTION', '1024')}
- **视图数**: 1 (正视图)
- **渲染引擎**: {os.environ.get('ENGINE', 'CYCLES')}
- **几何模式**: {'是' if '--geo_mode' in sys.argv else '否'}

---
*此文件由 render_batch_single_view.py 自动生成和更新*
"""
...
def main():
    ...
    total = len(inputs)
    print(f"[INFO] Found {total} models to render with single front view under {args.input_dir}")
    os.makedirs(args.out_root, exist_ok=True)
    
    # 设置进度文件路径
    if not args.progress_file:
        # 默认在输出目录下创建进度文件
        args.progress_file = os.path.join(args.out_root, "render_progress_single_view.md")
    ...
    for i, model_path in enumerate(inputs):
        ...
        # 直接输出到OUT_ROOT目录，文件名与输入相同但后缀为.png
        ...
        print(f"[{i+1}/{total}] Rendering single front view: {stem}")
        ...
        try:
            ...
            # 重命名渲染后的文件
            rename_rendered_files(out_dir, stem)
