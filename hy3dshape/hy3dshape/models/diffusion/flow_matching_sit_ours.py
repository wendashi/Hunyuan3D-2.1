...
class Diffuser(pl.LightningModule):
    ...
    def forward(self, batch):
        # 1. 提取 num_parts（新增）
        num_parts = batch.get('num_parts')  # [M,]，M 是 batch 中的 object 数量
        num_objects = num_parts.shape[0] if num_parts is not None else batch[self.first_stage_key].shape[0]
        ...
        # 4. 添加噪声（需要修改 timesteps 采样逻辑）
        noise = torch.randn_like(latents)
        
        # 按 object 数量采样 timesteps（不是 part 数量）
        # 注意：这里需要从配置中获取相关参数，暂时使用默认值
        u = torch.rand(num_objects, device=latents.device)  # 简化的均匀采样
        # 使用默认的 timesteps 数量
        num_timesteps = 1000  # 默认值
        indices = (u * num_timesteps).long()
        timesteps = torch.randint(0, num_timesteps, (num_objects,), device=latents.device)  # [M,]
        
        # 将 timesteps 按 num_parts 重复（参考 PartCrafter line 668）
        if num_parts is not None:
            timesteps = timesteps.repeat_interleave(num_parts)  # [N,]，N = sum(num_parts)
        
        # 计算 sigmas 并添加噪声
        # 使用简化的 sigmas 计算
        sigmas = torch.rand(num_objects, device=latents.device)  # [M,]
        if num_parts is not None:
            sigmas = sigmas.repeat_interleave(num_parts)  # [N,]
        
        # 扩展 sigmas 维度以匹配 latents
        ...
        # 5. CFG dropout（需要修改）
        # 注意：这里需要从配置中获取 cfg_dropout_prob，暂时跳过
        # if cfg_dropout_prob > 0 and num_parts is not None:
        #     # 按 object 生成 dropout mask，然后按 num_parts 重复（参考 PartCrafter line 675-678）
        #     dropout_mask = torch.rand(num_objects, device=device) < cfg_dropout_prob  # [M,]
        #     dropout_mask = dropout_mask.repeat_interleave(num_parts)  # [N,]
        #     # 应用 dropout mask
        #     ...
        
        # 6. 调用 DiT 模型（新增 attention_kwargs）
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            loss = self.transport.training_losses(
                self.model, 
                latents, 
                dict(contexts=contexts),
                num_parts=num_parts  # 传递 num_parts
            )["loss"].mean()
        return loss
