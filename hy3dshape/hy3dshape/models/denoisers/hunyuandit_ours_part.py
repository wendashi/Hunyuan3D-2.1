...
def _normalize_part_inputs(part_ids: torch.Tensor, global_mask: torch.Tensor = None):
    """
    Utility to sanitize part ids & derive default global token mask.

    Args:
        part_ids: Long tensor with shape (batch, seq_len) describing the per-token part id.
        global_mask: Optional bool tensor marking tokens that should bypass locality constraints.

    Returns:
        Tuple(part_ids_or_none, global_mask_or_none)
    """
    if part_ids is None:
        return None, None
    part_ids = part_ids.long()
    if global_mask is None:
        global_mask = part_ids.eq(-1)
    else:
        global_mask = global_mask.bool()
    return part_ids, global_mask


def build_part_attention_mask(
    part_ids_q: torch.Tensor,
    part_ids_k: torch.Tensor = None,
    global_mask_q: torch.Tensor = None,
    global_mask_k: torch.Tensor = None,
):
    """
    Build a boolean attention mask so that tokens only attend to the same part unless marked global.
    Returns shape (batch, seq_q, seq_k).
    """
    part_ids_q, global_mask_q = _normalize_part_inputs(part_ids_q, global_mask_q)
    part_ids_k, global_mask_k = _normalize_part_inputs(part_ids_k, global_mask_k)
    if part_ids_q is None:
        return None
    if part_ids_k is None:
        part_ids_k = part_ids_q
        global_mask_k = global_mask_q

    mask = part_ids_q.unsqueeze(-1) != part_ids_k.unsqueeze(-2)

    if global_mask_q is not None:
        mask = mask & (~global_mask_q.unsqueeze(-1))
    if global_mask_k is not None:
        mask = mask & (~global_mask_k.unsqueeze(-2))
    return mask
...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        attn_mask = build_part_attention_mask(part_ids_q, part_ids_k, global_mask_q, global_mask_k)
...
class Attention(nn.Module):
    ...
    def forward(self, x, part_ids: torch.Tensor = None, global_mask: torch.Tensor = None):
        B, N, C = x.shape
        ...
        attn_mask = build_part_attention_mask(part_ids, part_ids, global_mask, global_mask)
        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(1).expand(-1, self.num_heads, -1, -1)
...
class HunYuanDiTBlock(nn.Module):
    ...
    def forward(self, x, c=None, text_states=None, skip_value=None, attention_kwargs=None):
        ...
        part_kwargs = attention_kwargs or {}
        token_part_ids = part_kwargs.get("part_ids")
        token_global_mask = part_kwargs.get("global_part_mask")

        attn_out = self.attn1(
            self.norm1(x),
            part_ids=token_part_ids,
            global_mask=token_global_mask,
        )

        x = x + attn_out

        # Cross-Attention
        cross_part_ids = part_kwargs.get("context_part_ids")
        cross_global_mask = part_kwargs.get("context_global_mask")

        x = x + self.attn2(
            self.norm2(x),
            text_states,
            part_ids_q=token_part_ids,
            part_ids_k=cross_part_ids,
            global_mask_q=token_global_mask,
            global_mask_k=cross_global_mask,
        )
...
class HunYuanDiTPlain(nn.Module):
    ...
    def forward(self, x, t, contexts, attention_kwargs=None, **kwargs):
        ...
        part_cfg = {} if attention_kwargs is None else attention_kwargs.copy()
        part_ids = part_cfg.get("part_ids")
        global_part_mask = part_cfg.get("global_part_mask")
        if part_ids is not None:
            part_ids = part_ids.long()
            pad = torch.full(
                (part_ids.shape[0], 1),
                -1,
                dtype=part_ids.dtype,
                device=part_ids.device,
            )
            part_ids = torch.cat([pad, part_ids], dim=1)
            part_cfg["part_ids"] = part_ids
        if global_part_mask is not None:
            pad_mask = torch.ones(
                (global_part_mask.shape[0], 1),
                dtype=torch.bool,
                device=global_part_mask.device,
            )
            global_part_mask = torch.cat([pad_mask, global_part_mask.bool()], dim=1)
            part_cfg["global_part_mask"] = global_part_mask

        skip_value_list = []
        for layer, block in enumerate(self.blocks):
            skip_value = None if layer <= self.depth // 2 else skip_value_list.pop()
            x = block(x, c, cond, skip_value=skip_value, attention_kwargs=part_cfg)
