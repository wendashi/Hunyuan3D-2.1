...
class ImageConditionalFixASLDiffuserLogger(Callback):
    def __init__(
        ...
        max_size: int = None,
        save_dir: str = 'infer',
        batch_size: int = 2,  # 每个物体生成的 part 数量
        **kwargs,
    ) -> None:
        super().__init__()
        self.step_freq = step_frequency
        ...
        self.batch_size = batch_size  # 用于生成多个 part
    ...
    def on_train_batch_end(
        ...
        if pl_module.global_step % self.step_freq == 0:
            ...
            for image_path in image_paths:
                ...
                # 传入单个图像，通过 attention_kwargs 指定要生成的 part 数量
                # pipeline 会根据 num_parts 生成对应数量的 latents
                num_parts = self.batch_size
                    
                with torch.no_grad():
                    # 传入单个图像和 attention_kwargs
                    # pipeline 会根据 num_parts 生成对应数量的 part mesh
                    outputs = pl_module.sample(
                        batch={"image": image_path},  # 单个图像，不是列表
                        attention_kwargs={"num_parts": torch.tensor([num_parts])},
                        **self.kwargs
                    )
                    
                    # outputs 格式：[[mesh1, mesh2, ...]] 或 [[mesh]]
                    # sample 方法返回 [outputs]，其中 outputs 是 pipeline 的返回结果
                    pipeline_outputs = outputs[0]  # 获取 pipeline 的返回结果
                    
                    # pipeline 返回的可能是列表或单个 mesh
                    if isinstance(pipeline_outputs, list):
                        # 如果是列表，遍历保存每个 part 的 mesh
                        for part_idx, mesh in enumerate(pipeline_outputs):
                            if mesh is None:
                                continue
                            if isinstance(mesh, tuple) and len(mesh) == 2:
                                mesh = export_to_trimesh(mesh)
                            if isinstance(mesh, trimesh.Trimesh):
                                part_save_path = f"{base_save_path}_part{part_idx:02d}.glb"
                                os.makedirs(os.path.dirname(part_save_path), exist_ok=True)
                                mesh.export(part_save_path)
                    elif isinstance(pipeline_outputs, trimesh.Trimesh):
                        # 如果只返回单个 mesh，保存为原来的方式（兼容旧代码）
                        os.makedirs(os.path.dirname(save_path), exist_ok=True)
                        pipeline_outputs.export(save_path)
